...
בשני הסוגים הראשונים (התאמה/הדגמה וסיכום והתחלת עבודה) הסיכום הוא **העברת מקל
לצוות ההטמעה ושירות הלקוחות**. ברגע שעסקה נסגרת, הם בונים את המערכת רק לפי המסמך
הזה — בלי להאזין להקלטה. לכן הם חייבים לדעת גם מה הלקוח ביקש **וגם מה הוא במפורש
לא ביקש / לא צריך**, אחרת בונים דברים מיותרים או מפספסים ציפייה. פרט בהתאם.
...
1. **פגישת התאמה / הדגמה** — שיחה ראשונה עם לקוח פוטנציאלי:
   ...
   - **מה הלקוח לא ביקש / הבהיר שאינו צריך** — כל דבר שהלקוח דחה, אמר שלא
     רלוונטי לו, או שהובהר שאינו נכלל. מונע מההטמעה לבנות מיותר או לצפות לא נכון
...
2. **פגישת סיכום והתחלת עבודה** — הלקוח סוגר ומתחילים:
   ...
   - **מה הלקוח ביקש** — כל בקשה, בציטוט, שההטמעה צריכה לספק
   - **מה הלקוח לא ביקש / הובהר שאינו כלול** — כדי שלא ייווצר פער בין מה שהלקוח
     חושב שקנה לבין מה שסוכם בפועל
...
7. **פגישת תמיכה טכנית** — לקוח קיים שיש לו תקלה/שאלה על המערכת הקיימת:
   - **התקלה או הבקשה כפי שהלקוח תיאר אותה** — בציטוט: מה לא עבד, ממתי,
     ובאיזה חלק של המערכת. זה נקודת הפתיחה שאסור לרכך
   - **מה אובחן כגורם** — הסיבה בפועל, אם זוהתה; אם לא — כתוב שלא זוהתה
   - **מה תוקן / שונה במערכת בפועל** — רכיב-רכיב, כולל מה נבדק ואומת שעובד
   - **מה נשאר פתוח / לא נפתר** — כל תקלה שלא נסגרה בפגישה היא **חוב פתוח
     ללקוח**; ציין במפורש מה עוד דורש טיפול או בדיקה מול גורם נוסף
   - **מה נדרש מהלקוח** — גישות, מידע, החלטות, או פעולה מצידו כדי להשלים
   - **מה הוסבר / הודרך ללקוח** — כדי שיוכל להתמודד לבד בפעם הבאה
   - הצעד הבא, והאם צריך הסלמה לגורם נוסף (גד / הטמעה / ספק חיצוני)

8. **אחר / פנימית / לא ברור** — אם זו לא אחת מהפגישות שלמעלה, כתוב את סוג
