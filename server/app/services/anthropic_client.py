...
סוג הפגישה: <מכירה/הדגמה | אפיון | הקמה/הטמעה | המשך/תמיכה | פנימית | לא ברור>
...
כתוב אותו לפי סוג הפגישה שזיהית:

**מכירה/הדגמה** — מה שנלמד על הלקוח כאן קובע מה נבנה לו אחר כך:
...
- **מה הלקוח ביקש שיהיה לו במערכת** — רשימה מפורטת, בקשה-בקשה. גם בקשות
  שנאמרו אגב אורחא. זה החלק שאסור לפספס ממנו כלום
- מספרים שנאמרו: מחירים, תקציב, כמויות, לוחות זמנים
- התנגדויות וחששות, ומה נענה עליהן
- **מה הובטח ללקוח** — כל הבטחה, עם ציטוט מדויק
- הצעד הבא שסוכם

**אפיון** — המטרה שאפשר יהיה לגזור מכאן מסמך אפיון בלי להאזין להקלטה:
- תהליכים ופייפליינים: שלבים, שמות, מה מזיז ליד משלב לשלב
- שדות ונתונים שצריך לאסוף
- אוטומציות: טריגר → פעולה, לכל אחת
- הודעות ותכנים: מה נשלח, מתי, בערוץ מה, ונוסח אם נאמר
- אינטגרציות וחיבורים למערכות חיצוניות
- משתמשים והרשאות: מי רואה מה
- מה נשאר פתוח / דורש החלטה של הלקוח

**הקמה/הטמעה**:
- מה הוקם והוגדר בפועל בפגישה
- מה עדיין לא הוקם ולמה
- מה נדרש מהלקוח כדי להתקדם (חומרים, גישות, החלטות)
- תקלות שהתגלו ואיך טופלו
- מה הודרך והוסבר ללקוח

**המשך/תמיכה**:
- מה נפתר ומה נשאר פתוח
- בקשות חדשות שעלו
- **פערים בין מה שהובטח בעבר לבין המצב בפועל** — אם הלקוח מזכיר משהו שביקש
  קודם, צטט אותו במדויק

**פנימית / לא ברור** — כתוב "אין פירוט נוסף" ואל תמציא.
